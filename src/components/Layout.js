...
const Layout = () => {
  ...
  const { userProfile, dropModal, navMenu, setNavMenu, setDropModal,acctMenu, setAcctMenu, signOutModal, setSignOutModal, commentModal, setCommentModal, editModal, setEditModal } = useStateAuth();
    ...
    <div className="flex" onClick={pageHandler} >
      ...
      <div className="w-full md:flex-1 relative">
        ...
        {!isChatPath() && <div className="sticky top-0 left-0 w-full h-[3.5rem] border-b-[.1rem] bg-white/95 z-[40] flex items-center justify-center px-3">
          {!(location.pathname === '/') && <button onClick={()=>navigate(-1)} className = 'absolute left-2 text-black/60 hidden s500:flex'><ArrowBackIos className=""/></button>}
          <span className="absolute left-2 flex s500:hidden">
          {!(location.pathname === '/') ? <button onClick={()=>navigate(-1)} className = ' text-black/60 '><ArrowBackIos className=""/></button> : 
            <button className=""><Avatar src = {userProfile?.avatar}/></button>
          }
          </span>
          ...
          <button className="text-black/50 s500:hidden absolute right-2" onClick = {showNavMenu}><UilApps/></button>
        </div>}
